...
The ninth one has 1 more shift and 19 more whales in V1 than V2. 
...
V2 has two more whales than V1
...
# third shift has two more whales in V2. Observer IDs are different 40 (V1) vs. 42 (V2)
...
out.shift.3$data %>%
  ...
  summarise(max.n = max(V9),
            last.n = last(V9),
            d.max.last = max(V9) - last(V9)) -> data.summary.3.v2
# It's not anything to do with max vs last... 

# Get the raw data files
data %>% filter(begin >= tmp.V1[3,"begin"] & begin <= tmp.V1[3, "end"]) %>%
#data.3.5 %>% 
  filter(V2 == "S") %>%
  select(V5, V9) %>%
  mutate(V5 = as.numeric(V5), V9 = as.numeric(V9)) %>%
  ...
  summarise(max.n = max(V9),
            last.n = last(V9),
            d.max.last = max(V9) - last(V9)) -> data.summary.3.v1

# They are the same... 21. So, why did V1 ended up with 19?
```
