...
dir.name <- "Formatted Data Files for 2019 Analysis/"
#Watch start times, as fraction of a day
begin <- as.matrix(read.table(paste0(dir.name, "Data/begin.txt"), 
                              header=T))

#watch end times
end <- as.matrix(read.table(paste0(dir.name, "Data/end.txt"),
                            header=T))

#whale counts
n <- as.matrix(read.table(paste0(dir.name, "Data/n.txt"),
                          header=T))
...
# n1 <- as.matrix(read.table(paste0(dir.name, "Data/n1.txt"),
#                            header=T))      #These aren't needed; used in the WinBUGS GUI version previously. Here we can just re-use n
# dim(n1) <- c(179,2,6) #convert this back to a 3D array
# 
# 
# n2 <- as.matrix(read.table(paste0(dir.name, "Data/n2.txt"), 
#                            header=T))      #These aren't needed
# dim(n2) <- c(179,2,6) #convert this back to a 3D array


u <- as.matrix(read.table(paste0(dir.name, "Data/u.txt"),
                          header=T))
...
N_inits <- as.matrix(read.table(paste0(dir.name, "Initial Values/N_inits.txt"),
                                header=T))
