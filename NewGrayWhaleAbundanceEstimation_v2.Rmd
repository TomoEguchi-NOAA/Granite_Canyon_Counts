...
Visual surveys of the ENP gray whale migration at Granite Canyon, CA, have been conducted since the 1967/1968 season. Over this time series, a series of modifications to data collection methods were implemented. Notably, starting in the 2010/2011 season, observers began using a computer program to predict the movements of gray whale groups, increasing the likelihood of repeatedly observing the same groups and more precisely determining group sizes.

Because of these methodological shifts, Durban et al. (2015) treated data prior to the 2006/2007 season as incompatible with their model. Consequently, all subsequent analyses using their framework have been restricted to estimating abundance only from the 2006/2007 season onward (e.g., [@stewart_abundance_2021]; [@eguchi2023; @eguchi2024; @eguchi_eastern_2022; @eguchi2025]).
...
To compare the three approaches (Laake et al., Durban et al., and the new approach), I fitted generalized additive models (GAMs) to abundance estimates between the 2006/2007 and 2025/2026 seasons. Laake et al.'s and the new approaches were compared by fitting GAMs to the point estimates over the entire timeseries from the 1967/1968 to 2025/2026 seasons. The rate of recent decline in the estimated abundance was compared by fitting linear models to the abundance estimates between the 2015/2016 and 2024/2025 seasons.
...
Posterior distributions of the Richards equation parameters provided valuable insights into how annual gray whale migration varied among years. The $P$ parameters, which define the timing of the migration peak, shifted over the years (Figure \@ref(fig:P-posteriors)), a phenomenon reported in previous studies [@rugh_timing_2001], Eguchi et al. 2023, 2024, 2025), although the significance and causes for the changes are unknown. The $S_2$ parameter, which define the increasing part of the migration curve, also varied annually, indicating that the way the number of migrating whales increased within a season fluctuated from year to year (Appendix Figure A\@ref(fig:S2-posteriors)). On the other hand, the best model included only one common $S_1$ parameter among all seasons, indicating the decline of the number of migrating whales within a season was similar among years (Appendix Figure A\@ref(fig:S1-posteriors)), or insufficient counts were available towards the first half of migration.
...
The parameter $S_2$ defines how the curve increases to its peak. The rate of increase slows down as $S_2$ becomes larger (Figure A\@ref(fig:Figure-S2)). Because we make an assumption that there are no whales migrating before day 1, the upper bound of $S_2$ can be restricted to a certain value (e.g., $0 < S_2 < 5$).
