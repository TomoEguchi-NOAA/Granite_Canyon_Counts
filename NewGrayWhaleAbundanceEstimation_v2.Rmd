...
Using the observation that some parameters are temporary auto-correlated, I defined some parameters to be auto-correlated (Table). This decreased the number of parameters to be inferred from the data. 
...
Since the method of Durban's was published, various numerical tools were developed to assess convergence of MCMC. I applied the same tools to WinBUGS output from Durban et al.'s model. I found that the minimum bulk and tail ESS were too low (min(bulk ESS) = `r signif(min(ESS.bulk.BUGS$ess_bulk, digits = 2))`, min(tail ESS) = `r signif(min(ESS.tail.BUGS$ess_tail, digits = 2))`) to assume successful convergence of Markov chains. Consequently, the inference from this model are questionable. The rank-normalized Rhat statistics also indicated that convergence was not reached in `r length(BUGS.Rhat.big)` of `r dim(WinBUGS.out$BUGS.out$sims.array)[3]` monitored parameters (rank-normalized Rhat \> 1.01). 
...
No data point was flagged using the new method as lack-of-fit based on Pareto-k statistics indicated that all data points fit well to the model. This indicated that the model, i.e., combining the Richards equation and the hierarchical state-space modeling framework, was appropriate for analyzing the entire dataset.
