# Runs Posterior Predictive Checks on the selected model:
# 
...
best.model.name <- "M1a2_1gamma_mod5_ZI2_ShapeDev1"

best.model.fit <- readRDS(paste0("RData//Richards_HSSM_", 
                                 best.model.name,
                                 "_stan.rds"))

best.model.info <- readRDS(paste0("RData//Richards_HSSM_", 
                                 best.model.name,
                                 "_stan_info.rds"))
stan.data <- best.model.info$stan.data
...
ppc.res <- ppc_richards(best.model.fit, stan.data, n_draws = 1500)
ppc.res$pvalues
ppc.res$autocorr
ppc.res$plots$coverage
ppc.res$plots$obs_pred
ppc.res$plots$resid_day
ppc.res$plots$resid_fit
ppc.res$plots$dens
ppc.res$plots$resid_season

# More analysis on these
