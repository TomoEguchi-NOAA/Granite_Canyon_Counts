...
# The last shift (6) has one more whale in V2 than in V2
Data_Out.v2 %>% 
  filter(time.steps == difs.1[idx, "time.step"]) 
...
Two more whales in V2 than in V1.
...
# The first shift has 2 more whales in V2 than in V1
Data_Out.v2 %>% 
  filter(time.steps == difs.1[idx, "time.step"]) 
...
# They add up to 16, which is more than V2 found... 

# V2 has no group 8 (but only 1 whale) - not sure why they were added to 13. The
# group 8 was sighted in the next shift, so it should not be added in this 
# shift. So, adding groups from 1 to 7 should be 15, not 13. Last and maximum
# counts per group did not make any difference. 
```
...
One more shift and 4 more whales in V1 than in V2.
...
# Shift 1 was too short and VS was 5 with no whales. 
# Shift 2 somewhat matches with first shift in V1 but duration is different
#  Shifts 3 to 6 seem the same as V1's shifts 2-5.
...
out.shift.2$data %>%
  ...
  group_by(V5) %>%
  summarise(last.n = last(V9),
            max.n = max(V9)) -> summary.30.2.V2

data %>% 
  filter(begin >= tmp.V1[1,"begin"] & begin <= tmp.V1[1, "end"]) %>%
  filter(V2 == "S") %>%
  select(V5, V9) %>%
  mutate(V5 = as.numeric(V5), V9 = as.numeric(V9)) %>%
  group_by(V5) %>%
...
# This shift started at 10:00 and ended at 10:27:39
# In the data, start time was changed to 9:00:00 (71.3750). In the raw data file,
# the day started at 7:30 but due to poor conditions (BF = 4, VS = 5), observation 
# did not start until 10:00:26. 
