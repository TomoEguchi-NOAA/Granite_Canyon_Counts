...
# 2023-03-23 and later are with two stations

run.date <- Sys.Date() #"2023-05-26" #"2023-04-14" #Sys.Date() #"2022-11-08" #   

model.v <- "v1" # "v3"
model.d <- "logN_bino_"
#out.file.name <- paste0("RData/Pois_Binom_sim_results_", run.date, ".rds")
#out.file.name <- paste0("RData/Pois_Binom_v2_sim_results_", run.date, ".rds")
#out.file.name <- paste0("RData/Pois_Binom_", model.v, "_sim_results_", run.date, ".rds")
out.file.name <- paste0("RData/", model.d, model.v, "_sim_results_", run.date, ".rds")
...
if (!file.exists(out.file.name)){
  ...
  #jags.model <- paste0("models/model_Richards_pois_bino_", model.v, ".txt")
  jags.model <- paste0("models/model_Richards_", model.d, model.v, ".txt")
...
if (model.v == "v3"){
  ...
  params.to.summarize.n.Season <- c("S1", "S2", "Max")  
  # Some are common among all years
  params.to.summarize.n.1 <- c("mean.prob", "BF.Fixed", "VS.Fixed", "K", "P")

}

# Add year specific indices if exist
...
params.df <- data.frame(names = param.names,
                        value = c(NA, simulated.data$sim.parameters$S1,
                                  ...
                                  NA, simulated.data$sim.parameters$K,
...
for (k in 1:length(param.names)){
  save.fig.fcn(p.trace.sim.JAGS[[k]] +
                 geom_hline(aes(yintercept = params.df[k, "value"]),
                            color = "red", size = 1.2),
               file.name = paste0("figures/", param.names[k], "_sim_trace_JAGS_", model.d, model.v, "_",
                                  dpi.set, "dpi.png"),

               dpi = dpi.set, replace = save.fig)

  save.fig.fcn(fig = p.dens.sim.JAGS[[k]],
               file.name = paste0("figures/", param.names[k], "_sim_dens_JAGS_", model.d, model.v, "_",
                                  dpi.set, "dpi.png"),
               dpi = dpi.set, replace = save.fig)
...
save.fig.fcn(p.mean.N.hats.sim.JAGS,
             file.name = paste0("figures/mean_N_sim_JAGS_",model.d,  model.v, "_", dpi.set, "dpi.png"),
             replace = save.fig,
             dpi = dpi.set)

if (save.fig)
  ggsave(p.mean.N.hats.sim.JAGS,
         filename = paste0("figures/mean_N_sim_JAGS_",  model.d, model.v, "_", dpi.set, "dpi.png"),
...
save.fig.fcn(fig = p.N.hats.sim.JAGS,
               file.name = paste0("figures/N_hats_sim_JAGS_", model.d,  model.v, "_", dpi.set, "dpi.png"),
               dpi = dpi.set, replace = save.fig)
...
save.fig.fcn(fig = p.trace.all.sim.JAGS,
             file.name = paste0("figures/trace_plots_sim_JAGS_", model.d,  model.v, "_", dpi.set, "dpi.png"),
             dpi = dpi.set, replace = save.fig)
...
knitr::include_graphics(paste0("figures/trace_plots_sim_JAGS_",model.d, model.v, "_", dpi.set, "dpi.png"))
...
knitr::include_graphics(paste0("figures/mean_N_sim_JAGS_", model.d,  model.v, "_", dpi.set, "dpi.png"))
```
...
knitr::include_graphics(paste0("figures/N_hats_sim_JAGS_", model.d,  model.v, "_", dpi.set, "dpi.png"))
```
...
# With new simulated data (two stations for the first year), unobserved real came back... reduced the number of steps to 60000 with burn in of 30000 and it ran okay. 
