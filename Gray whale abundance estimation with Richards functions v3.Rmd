...
lm.Eguchi <- lm(log(Nhat) ~ time, 
                data = all.estimates.Eguchi %>% filter(start.year > 2013))

lm.Laake <- lm(log(Nhat) ~ time, 
               data = all.estimates.Laake %>% filter(start.year > 2013))

lm.Durban <- lm(log(Nhat) ~ time, 
                data = all.estimates.Durban %>% filter(start.year > 2013))

```


Fitted GAMs to the estimated abundance between the 2006/2007 and 2024/2025 seasons indicated that the three approaches provided comparable results with respect to the general trend of abundance (Figure \@ref(fig:Figure-gam-fits)). Furthermore, linear models ($log(Nhat) = time + e$) fitted to the data from the 2014/2015 season to the 2024/2025 season indicated a similar rate of decline from the three approaches (Laake: `r signif(lm.Laake$coefficients[2],1)`. Durban: `r signif(lm.Durban$coefficients[2],1)`, Eguchi: `r signif(lm.Eguchi$coefficients[2],1)`)
