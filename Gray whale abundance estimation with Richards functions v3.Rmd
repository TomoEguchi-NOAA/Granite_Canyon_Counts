...
for (k in 1:6){
  ...
  new.Rhat <- rank.normalized.R.hat(.out[[k]]$jm$samples, params.2)
  #max.Rhat <- lapply(.out[[k]]$jm$Rhat, FUN = max, na.rm = T) %>%
  #  unlist()
  max.Rhat.big[[k]] <- new.Rhat[which(new.Rhat > 1.01)]
...
The models were fitted to the sightings data over 32 seasons. According to the rank-normalized R-hat statistic, Model 4 showed the least number of parameters with the rank-normalized Rhat > 1.01, where the maximum was Y. For those parameters in Model 4 with Rhat>1.01, trace plots indicated satisfactory convergence for all of them (Appendix figure). Practically, however, Model 3 with year-specific P parameter made more sense than Model 4, which assumed a constant P parameter among years. Consequently, I use Model 4 for parameter inference in the following section. 

The Pareto-k statistics indicated that `r which(LOOIC.n[[4]]$loo.out$diagnostics$pareto_k > 0.7) %>% length()` data points did not fit well to the model, which was `r LOOIC.n[[4]]$loo.out$` % of all data points. For the remainder of the data points, Pareto k statistics was \< 0.7, indicating good fit.
