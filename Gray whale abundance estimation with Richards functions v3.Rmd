...
```{r model-comparison, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
```{r JAGS-results, echo=F,message=FALSE, cache=TRUE}
rm(list = c(".out"))
ver <- 5
...
```{r posterior-Richards-plots, echo=FALSE, message=FALSE, cache=TRUE}
...
```{r posterior-fixed-effects-plots, echo=FALSE, message=FALSE, cache=TRUE}
...
```{r gam-glm-fit, echo=F,message=FALSE, cache=TRUE}
library(mgcv)
...
#### Trace plots of parameters with high rank-standardized R hat values
...
#### Changes in S1 and S2 over time
