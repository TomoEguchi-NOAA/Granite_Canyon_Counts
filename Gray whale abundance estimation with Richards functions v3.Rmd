...
## A New Modeling Approach {.unnumbered}

A primary limitation of using a spline is the difficulty in handling missing data points (e.g., weekends, seasons with fewer than 90 observation days), which are linearly interpolated unless there is an assumed common data-generating model among years, such as the Gaussian distribution used by Durban et al. To address this and other limitations, I propose a new approach that replaces the Gaussian distribution and spline functions with a single, more flexible function.
...
where $d > 0$ is the number of days since an arbitrary start date, $S_1 > 0$ and $S_2 > 0$ define how the slope decreases and increases, respectively, $K > 0$ defines the "flatness" at the peak of the function, $P_1$ and $P_2$ ($P_1 <= P_2$) define where the peak is relative to the range of $d$, where $min(d) < P_1 <= P_2 < max(d)$, $N_{min}$ is zero, i.e., the number of whales migrating outside of a migration season and, $N_{max} >> N_{min}$. $N_{max}$ is not the maximum number of whales migrating per day but it is a parameter that may be fixed or estimated during the analysis. Effects of these parameters on the shape of the function are described in the Appendix.
...
The new approach was used to analyze the entire dataset from the 1967/1968 season onward, and the results were compared to the estimates reported in Eguchi et al. (2025). Sightings data from the 1967/1968 and 2005/2006 seasons were obtained from the ERAnalysis package (<https://github.com/jlaake/ERAnalysis>, Laake 2012). An R script was written to convert the sightings data in the package to an input structure required for the new analysis. No raw data were available for the 2006/2007 season, so it was excluded from this analysis. Data from 2007/2008 to 2024/2025 were extracted from raw data files using another R script (available at the GitHub repository). All shifts of at least 60 minutes with the Beaufort sea state of \< 5 and the visibility code of \< 5 were included in the analysis. Laake et al.’s approach was applied to the entire dataset after formatting new data from the 2009/2010 season into the input object for Laake et al.’s approach.
...
The eight models were fitted to the sightings data over 32 seasons. Based on LOOIC and Pareto-k statistics, Model `r out.table$model[which(out.table$dLOOIC == 0)]` was identified as the best-fitting model (Table \@ref(tab:model-table)). Although Model `r out.table$model[which(n.big.Rhat == min(out.table$n.big.Rhat))]` had the fewest parameters with an R-hat statistic \< 1.01 (Table 2), after visual inspection of trace plots for parameters with high Rhat statistics (Appendix Figures \@ref(fig:Figure-trace-1), \@ref(fig:Figure-trace-2), and \@ref(fig:Figure-trace-3)), I concluded that Model `r out.table$model[which(out.table$dLOOIC == 0)]` was the most appropriate for inference. The model consists of year-specific $P$ ($P_1 = P_2$), and year-specific $S_1$ and $S_2$ parameters (Table \@ref(tab:model-definitions)).
