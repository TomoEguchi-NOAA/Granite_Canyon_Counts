...
### Some problems with Durban et al's method {.unnumbered}

Durban et al. (2015) used the "cut" function within WinBUGS to dissociate estimated parameters in one function (i.e., $f_{d,t}$ or $g_{d,t}$) from those in the other function (i.e., $g_{d,t}$ or $f_{d,t}$, respectively). The function (*cut*) is unavailable in modern Bayesian computation packages (e.g., JAGS, STAN) and its use has been questioned [@plummer_cuts_2015]. In short, the "cut" function does not converge to a well-defined limiting distribution (Plummer 2015). In addition, the assumption that the number of gray whales migrating in front of the observation station follows a Gaussian distribution centered around the midpoint of the pre-defined migration season is somewhat questionable. The actual curve in abundance may not be symmetric around a peak, and the peak may not be instantaneous. In other words, the peak may persist for a few days. Fitting spline functions to observed counts may alleviate some of these problems, but a fitted spline to observed counts may be too flexible. A spline fit would lose the general idea that the number of whales increases from the beginning of a migration season, reaches a peak, then decreases over time to the end of the migration season. In addition, Durban et al.'s approach implicitly assumed that two observation stations operated simultaneously with identical effort. In reality, however, watch durations were not identical between the two stations in previous years (i.e., data in Laake's analysis).

## Modification {.unnumbered}

Because of the way a spline is fit to data points, it is impossible to use just a spline when the data are not collected from day 1. Missing data points (e.g., weekends, not having 90 days of observations) are linearly interpolated unless there is an assumed data generating model (the Gaussian distribution in the approach by Durban et al.) that is common among years. Instead of a Gaussian distribution and spline functions, I use one flexible function.

### Model description {.unnumbered}

In the new approach, I use a more flexible function that can accommodate the general shape (increase, peak, then decrease) and is asymmetrical around the peak. This pattern can be modeled with the product of two sigmoid equations or mathematical functions with a peak in the middle, but not necessarily halfway. Durban et al. used a Gaussian function to model this pattern. A similar pattern is often found in nesting marine turtles. For example, the first-order derivative of a modified form of the Verhurst equation to model was used to model the number of nesting leatherback turtles in French Guiana and Suriname [@girondot_modeling_2007]. The equation is similar to the Richards equation [@richards1959] and has the following form.
...
$S_1 > 0$ and $S_2 > 0$ define how the slope decrease and increases, respectively,
...
$N_{max} >> N_{min}$. $N_{max}$ is not the maximum number of whales migrating per day but it is a parameter that may be fixed or estimated during the analysis. Effects of these parameters on the shape of the function are described in the Appendix.

```{r model-definitionws, echo=FALSE, message=FALSE}
...
I call this function the Richards equation hereafter. I created eight versions of the equation, based on how $P_1$, $P_2$, $S_1$, and $S_2$ were defined with respect to time (Table \@ref(tab:model-definitions)). The $N_{max}$ parameter was assumed to be year specific and $K$ constant among all years. Each parameter had a hyper-distribution of either a Gaussian, uniform, or gamma distribution. Uniform distributions were used for the prior distributions of hyperparameters (Appendix Table \@ref(tab:hyperparameter-table)).
...
The Richards equation provided the daily mean number of gray whales passing through the observation point. The daily number of whales passing through the observation point was modeled with a Poisson distribution with the mean.

$$ N_{t, y} \sim POI(\bar{N}_{t,y}) $$

where $\bar{N}_{t,y}$ is the expected number of whales that migrate through the sampling area on the $t$-th day of the season $y$, which is modeled with the Richards equation above.

The N-mixture model of Durban et al. was used to approximate the sampling process. Specifically, the observation probability $p_{i_t, s, y}$ was a function of the viewing conditions (the Beaufort sea state ($B$) and visibility code ($V$)), the overall mean probability of detection ($\beta_0$), observer random effects ($O$), and a watch duration offset ($L$). The watch duration offset was included to accommodate differing durations of observation shifts due to inclement conditions. For the observer random effects, because some observers lacked multiple observation periods, they were not useful as a random effect in the model. Consequently, I pooled all observers with less than 10 sightings as one “observer.”

$$ logit(p_{i_t, s, y}) = \beta_0 + \beta_{B} * B_{i_t, y} + \beta_{V} * V_{i_t, y} + O_{i_t, s, y} + log(L_{i_t, y})  $$ where the index $i_t$ is the i-th watch period of the t-th year, $s$ is the station (1 or 2), and $y$ is the season.
...
In Durban et al’s analysis, the annual migration started on December 1 and ended 90 days later, where the number of passing whales was assumed to be zero on days 1 and 90 (February 29th or March 1st). In some years, however, observations were made beyond March 1st. To analyze all data, I defined the annual migration period to be 100 days (March 10th or 11th), rather than 90 days, starting on December 1, and assumed that the number of passing whales was zero on days 1 and 100 ($max(d) = 100$).

The entire dataset from the 1967/1968 season was analyzed using the new approach and the results compared to the reported estimates in Eguchi et al. (2025). Sightings data between the 1967/1968 and 2005/2006 seasons were obtained from the *ERAnalysis* package (<https://github.com/jlaake/ERAnalysis>, Laake 2012). An R script was written to convert the sightings data in the package to an input structure for the new analysis. No raw data were available for the 2006/2007 season. Consequently, it was excluded from the analysis. The remainder of the data, i.e., 2007/2008 to 2024/2025, was extracted from raw data files using an R script (available at the GitHub repository). All sighting data were used to create an input object for the new analysis, where all shifts of at least 60 minutes with the Beaufort sea state of \< 5 and the visibility code of \< 5 were included in the analysis. Laake et al.'s approach was applied to the entire dataset after formatting new data from the 2010/2011 (?) season to the input object for Laake et al.'s approach.

The new models were fitted to the data using JAGS (Plummer) in the R statistical environment, using the *jagsUI* package (Kellner 2024). For each model, 550,000 steps of five chains were run, where the first 500,000 samples of each chain were treated as “burn-in” and discarded. The remaining 250,000 posterior samples (50,000 samples of five chains) were thinned by taking every 100 samples, resulting in 2500 posterior samples for making inference. The convergence of MCMC was determined through the rank-normalized R-hat statistic [@vehtari2021]. Goodness-of-fit was determined via Pareto-k statistics [@vehtari_practical_2017] , using the *loo* package (Vehtari et al. 2024).

To compare the three approaches (Laake et al.’s, Durban et al.’s, and the new), I fitted generalized additive models to point abundance estimates between the 2006/2007 and 2024/2025 seasons. Raw data were not available for the 2007/2008 season, so it was not included in Laake’s and the new analyses. Further, the rate of recent decline in the estimated abundance was compared among the three approaches by fitting linear models to the point abundance estimates between the 2015/2016 and 2024/2025 seasons.
...
Estimated abundances from the new approach generally followed the same trend as the previous analyses, except for a few notable differences (Figure \@ref(fig:Figure-N-hats)). Laake’s and the new approaches exhibited lower estimates than those from Durban’s approach, where the new approach resulted in the lowest in most years. For the majority of the estimates, credible and confidence intervals overlapped significantly. The exact abundance estimates, however, are not critical because they do not reflect the total abundance of the population. The three approaches exhibited similar patterns and fluctuations in estimated abundance.
...
Posterior distributions of the parameters of the Richards equation provided valuable insights into how annual gray whale migration varied among years. The P parameters, which defined the location of the peak, changed over the years (Figure \@ref(fig:Figure-P-posteriors)). This has been reported in previous studies (Eguchi et al. 2023, 2024, 2025), although the significance and causes for the change are unknown. The S1 and S2 parameters, which define the width of the function, also varied among years, indicating that the duration of migration varied among years (Appendix Figures \@ref(fig:Figure-S1-posteriors) and \@ref(fig:Figure-S2-posteriors)).
...
As expected, the fixed effect of the sighting condition (i.e., the visibility code) on observation probability was negative (Figure \@ref(fig:Figure-Fixed-posteriors)), indicating that less favorable conditions resulted in lower detection probabilities. The fixed effect of the Beaufort sea state on observation probability was positive (Figure \@ref(fig:Figure-Fixed-posteriors)) indicating that higher Beaufort sea states corresponded with greater detection probabilities. This was a perplexing result. More singleton whales might have been detected at low sea states than at higher sea states, whereas only large groups were sighted at high sea states. At the Beaufort sea state 0, the largest observed group was `r n.and.bf %>% filter(bf == 0) %>% summarise(max.n = max(n))` whales, whereas for the Beaufort sea state \> 0, `r n.and.bf %>% filter(bf > 0, n > 74) %>% summarise(n.more.than.74 = n())` groups were greater than 74 whales . The observer random effects indicated significant differences among observers, as found in the previous analysis by Durban et al. (Appendix Figure \@ref(fig:Figure-observers)).
...
The new approach yielded comparable results to those obtained by Laake et al.’s and Durban et al.’s approaches. Although point estimates were different in some years, confidence/credible intervals overlapped in the majority of the time series. The new approach included several favorable properties compared with Durban et al.’s approach. First, it eliminated the use of the “cut” function, which had been deemed statistically inappropriate (Plummer yr). Second, it is coded in a more contemporary language (JAGS), which is widely used and freely available. Third, the goodness-of-fit is determined through a modern criterion using log-likelihood (LOOIC and Pareto-k statistic). In Durban’s approach, only one model was available, and the goodness-of-fit was not evaluated. Fourth, it runs faster than WinBUGS. The most recent analysis over the past 11 years using WinBUGS with 100,000 MCMC steps took `r signif(WinBugs.out$Run_Time, 3)` hrs. In comparison, the entire 32-year dataset was analyzed in approximately `r signif(jm.out$Run_Time, 2)` hrs on the same computer using the new approach with 550,000 MCMC steps of five chains using JAGS. Fifth, although the general modeling framework remains the same N-mixture model, the new approach is more straightforward.

Examination of Pareto-k statistics indicated that high Pareto-k values coincided with extreme data points, where counts were higher or lower than neighboring counts (Appendix Figure \@ref(fig:Figure-pareto-k)). Although statistically alarming, these deviations among daily counts are consistent with the gray whale migration. Consequently, I concluded that the model itself, i.e., the Richards equation and N-mixture model, was appropriate for the entire dataset.
...
The new approach can accommodate additional information if it becomes available. For example, there is an effort to estimate the precision and possible biases of estimated group sizes by visual observers, using uncrewed aircraft systems (UAS). Suppose a correction function can be determined for each sighting from the UAS-based data, conditional on the recorded group size by visual observers. Such a function can be incorporated into the observation likelihood loop, where each sighting within a period is corrected using the function. Finally, the model can be incorporated into a holistic population model, such as one created by [@stewart_boom-bust_2023].

In summary, the new approach provided a simpler and faster algorithm to estimate the abundance of gray whales from the long-term visual surveys conducted at Granite Canyon, CA.
...
```{r Figure-S1-posteriors, echo=FALSE, message=FALSE, fig.cap="Changes in the S1 parameter of the Richards equation over time. S1 defines the wideness of the right side of the curve."}

knitr::include_graphics(paste0("figures/posteriors_S1_", dpi.set, "dpi.png"))

```

```{r Figure-S2-posteriors, echo=FALSE, message=FALSE, fig.cap="Changes in the S2 parameter of the Richards equation over time. S2 defines the wideness of the left side of the curve." }
