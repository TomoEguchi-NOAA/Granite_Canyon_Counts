...
create.WinBUGS.data <- function(in.data){
  ...
  BUGS.data <- list(n = BUGS.n,
                    ...
                    n.obs = length(unique(in.data.1$seq.ID))+1,
                    periods = periods,
                    obs = BUGS.obs,
                    vs = vs,
                    bf = bf,
...
Effort %>%
  ...
  select(Start.year, nwhales, effort, vis, beaufort, ID, dt) -> Durban.data

Durban.data$ID[is.na(Durban.data$ID)] <- 98

# Durban.data %>%
#   mutate(ID.char = Observer) %>%
#   left_join(Observer.1 %>% select(Initials, ID.char), by = "ID.char") %>%
#   mutate(ID = ID.char) -> Durban.data.1
  #mutate(obs = Observer.x) %>%
  #dplyr::select(-c(Initials, Observer.x, Observer.y, Name, Sex)) %>%
  #rename(ID.1 = ID) %>%
  #mutate(ID.char = obs) %>%
  #left_join(Observer.1, by = "ID.char") %>% #-> tmp
  #dplyr::select(-c(Initials, Observer, Name, Sex, ID.char)) 
...
max.dt <- 90
Durban.data %>%
  filter(dt <= max.dt) -> Durban.data.2
...
BUGS.data$n.knots <- length(BUGS.data$knot)
