...
The mean of the Poisson distribution was modeled in terms of the number of whales passing each day ($d$), with an offset for the effort duration of each watch period, $E_{d(j),t}$ in decimal days. This formulation was fine in Laake et al's approach as a GAM was fitted to observed counts. In Durban's approach, however, the mean of the Poisson distribution ($\lambda$) models the true abundance, which should not be a function of effort.  
