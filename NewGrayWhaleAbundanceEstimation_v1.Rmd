...
$$ logit(p) = logit(p_0) + BF_I \times B_{BF} \times BF + VS_I \times B_{VS} \times VS + O_I \times O $$

$$ obs.prob = u \times p $$

where $BF_I$, $VS_I$, and $O_I$ are indicator variables, $B_{BF}$ and $B_{VS}$ are coefficients, $BF$ and $VS$ are observed Beaufort and visibility codes, respectively, $O$ is the observer random effect, and $u$ is an indicator function on whether or not a secondary observer was present. Coefficients were computed separately for the normal and spline functions, where one or the other was selected based on which function was deemed appropriate for each day.
...
### Data {-}
A series of modifications were made from the 1967/1968 to 2006/2007 seasons (Laake et al. 2012). starting in the 2010/2011 season, further modifications were employed where a computer was used to predict movements of gray whale pods, thereby increasing the chance of repeatedly observing the same pods to accurately determine the number of individuals in each pod. In Durban et al. (2015), they treated the data from previous years as not compatible in their analysis. Consequently, the subsequent analysis to estimate gray whale abundance used the data since the 2006/2007 season (e.g., Stewart and Weller 2021, Eguchi et al., 2022, 2023, 2024, 2025). 

In this paper, however, I used all the data from the 1967/1968 season because similar data were collected throughout the survey. I developed the model to be able to accommodate differing survey duration.   

### New Modeling Approach {.unnumbered}

A primary limitation of using a spline in the Durban et al. approach was the difficulty in handling missing data points (e.g., weekends, seasons with fewer than 90 observation days). To address this and other limitations, I propose a new approach that replaces the Gaussian distribution and spline functions with a single flexible function.

This new approach uses a function that can accommodate an asymmetrical shape around the peak, while reflecting the general pattern of increase, peak, and decrease in whale counts. This pattern can be modeled with the product of two sigmoid equations or mathematical functions that have a peak that is not necessarily centered. A similar pattern is often found in nesting marine turtles. For instance, a model derived from a modified Verhust equation was used to describe the number of leatherback turtles in French Guiana and Suriname [@girondot_modeling_2007]. The equation is similar to the Richards growth function [@richards1959] and has the following form.
...
where $d > 0$ is the number of days since an arbitrary start date, $S_1 > 0$ and $S_2 > 0$ define how the slope decreases and increases, respectively, $K > 0$ defines the "flatness" at the peak of the function, $P_1$ and $P_2$ ($P_1 <= P_2$) define where the peak is relative to the range of $d$, where $min(d) < P_1 <= P_2 < max(d)$, $N_{min}$ is zero, i.e., the number of whales migrating outside of a migration season and, $N_{max} >> N_{min}$. $N_{max}$ is not the maximum number of whales migrating per day but it is a parameter that may be fixed or estimated during the analysis. Effects of these parameters on the shape of the function are described in the Appendix. Hereafter, this function will be referred to as the Richards equation. 

I created eight versions of the equation, based on how $P_1$, $P_2$, $S_1$, and $S_2$ were defined with respect to time (Table \@ref(tab:model-definitions)). The $N_{max}$ parameter was assumed to be year-specific and $K$ was assumed constant across all years. Each parameter had a hyper-distribution of either a Gaussian, uniform, or gamma distribution. Uniform distributions were used for the prior distributions of hyperparameters (Appendix Table \@ref(tab:hyperparameter-table)).
...
Observed counts were modeled with a binomial distribution:
...
The new approach was used to analyze the entire dataset from the 1967/1968 season onward, and the results were compared to the estimates reported in Eguchi et al. (2025). Sightings data between the 1967/1968 and 2005/2006 seasons were obtained from the ERAnalysis package (<https://github.com/jlaake/ERAnalysis>, Laake 2012). An R script was written to convert the sightings data in the package to an input structure required for the new analysis. No raw data were available for the 2006/2007 season, so it was excluded from this analysis. Data from the 2007/2008 season to the 2024/2025 season were extracted from raw data files using another R script (available at the GitHub repository). All shifts of at least 60 minutes with the Beaufort sea state of \< 5 and the visibility code of \< 5 were included in the analysis. Laake et al.’s approach was applied to the entire dataset after formatting new data from the 2009/2010 season into the input object for Laake et al.’s approach.

The new models were fitted to the data using JAGS (Plummer) in the R statistical environment, using the *jagsUI* package (Kellner 2024). For each model, five MCMC chains were run for 550,000 steps each. The first 500,000 samples of each chain were discarded as “burn-in.” The remaining posterior samples were thinned by taking every 100 samples, resulting in 2500 posterior samples for inference. The convergence of MCMC was assessed using the rank-normalized R-hat statistic [@vehtari2021]. Goodness-of-fit was determined via Pareto-k statistics [@vehtari_practical_2017] via the *loo* package (Vehtari et al. 2024).
...
The eight models were fitted to the sightings data over 32 seasons. Based on LOOIC and Pareto-k statistics, Model `r out.table$model[1]` was identified as the best-fitting model (Table \@ref(tab:model-table)). Although Model `r model.ID[which(n.big.Rhat == min(n.big.Rhat))]` had the fewest parameters with the R-hat statistics \< 1.01 (Table 2), after visual inspection of trace plots for parameters with high Rhat statistics (Appendix Figures \@ref(fig:Figure-trace-1), \@ref(fig:Figure-trace-2), and \@ref(fig:Figure-trace-3)), I concluded that Model `r out.table$model[1]` was the most appropriate for inference. The model consists of year-specific $P$ ($P_1 = P_2$), $S_1$, and $S_2$ parameters (Table \@ref(tab:model-definitions)).
...
Estimated abundances from the new approach generally followed the same trend as the previous analyses, with a few notable differences (Figure \@ref(fig:Figure-N-hats)). The new approach and Laake et al.’s approach produced lower estimates than Durban et al.’s approach in most years. For the majority of the estimates, the 95% credible and confidence intervals overlapped significantly.

```{r Figure-N-hats, echo=FALSE, message=FALSE, fig.cap = "Estimated annual abundance and their 95% CIs using Laake's method (green), Durban's method (red), and the new approach (blue)."}
...
The new modeling approach yielded results comparable to those obtained by the methods of Laake et al. and Durban et al. Although point estimates were different in some years, the confidence and credible intervals overlapped in most of the time series, and all three methods captured similar long-term trends and recent rates of decline.

The new approach offers several advantages over the method of Durban et al. First, it eliminates the use of the “cut” function, which has been identified as statistically inappropriate (Plummer 2015). Second, the model is coded in JAGS, a more contemporary and widely used Bayesian modeling language, which runs significantly faster than WinBUGS. The analysis of the entire 32-year dataset took approximately 6.5 hrs with the new approach, compared to 18.5 hours for an 11-year analysis using WinBUGS. Third, the new framework allows for formal model selection and goodness-of-fit assessment using modern criteria, such as LOOIC and the Pareto-k statistic, which were not part of the original Durban et al. analysis. Fourth, while retaining the N-mixture modeling framework, the new approach is more straightforward in its implementation.

Examination of Pareto-k statistics indicated that high Pareto-k values coincided with extreme data points, where counts were unusually high or low compared to adjacent shifts (Appendix Figure \@ref(fig:Figure-pareto-k)). While statistically notable, these fluctuations are consistent with the known patterns of gray whale migration. Therefore, I concluded that the model, i.e., combining the Richards equation and an N-mixture framework, was appropriate for analyzing the entire dataset.
