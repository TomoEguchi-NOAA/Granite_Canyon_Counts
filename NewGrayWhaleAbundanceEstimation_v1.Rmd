...
where the index $i_t$ is the watch period $i$ of the day $t$, $s$ is the station (1 or 2), and $y$ is the season. The intercept ($\beta_0$) in the new approach Equation \@ref(eq:detection-probability) can be expressed in terms of the intercept ($p_0$) of the Durban et al. approach \@ref(eq:detection-probability-Durban) via the logit function:
...
Examination of Pareto-k statistics indicated that high Pareto-k values coincided with extreme data points, where counts were unusually high or low compared to adjacent shifts (Appendix Figure `r officer::run_word_field("REF Figure-pareto-k \\h")`). While statistically notable, these fluctuations are consistent with the known patterns of gray whale migration. Therefore, I concluded that the model, i.e., combining the Richards equation and the N-mixture framework, was appropriate for analyzing the entire dataset.
...
The parameter $P$ defines the location of its peak (Figure `r officer::run_word_field("REF Figure-P \\h")`).
...
```
:::{custom-style="Image Caption"}
`r fig.S1.num` Changes in the $S_1$ parameter of the Richards equation over time. $S_1$ defines the wideness of the right side of the curve.
:::
...
```
:::{custom-style="Image Caption"}
`r fig.S2.num` Changes in the $S_2$ parameter of the Richards equation over time. $S_2$ defines the wideness of the left side of the curve.
:::
