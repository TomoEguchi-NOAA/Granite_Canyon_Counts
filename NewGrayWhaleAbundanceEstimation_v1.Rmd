...
### Data {-}
A series of modifications were made from the 1967/1968 to 2006/2007 seasons (Laake et al. 2012). starting in the 2010/2011 season, further modifications were employed where a computer was used to predict movements of gray whale pods, thereby increasing the chance of repeatedly observing the same pods to accurately determine the number of individuals in each pod. In Durban et al. (2015), they treated the data from previous years as not compatible in their analysis. Consequently, the subsequent analysis to estimate gray whale abundance used the data since the 2006/2007 season (e.g., Stewart and Weller 2021, Eguchi et al., 2022, 2023, 2024, 2025). Since the 2010/2011 season, no dual-observer suvey has been conducted to estimate detection probability due to the logistical and financial constraints. Consequently, the data from the two seasons (i.e., 2009/2010 and 2010/2011 seasons) are the primary source of information for estimating detection probabilities. Prior to the 2006/2007 season, two observation stations were present in several years. In those years, sightings of the same pods by two teams were used to estimated detection probabilities via the "capture-recapture" method. These sightings data can be used in the N-mixture framework by retaining the number of observed pods but disregarding the pod identity. In this paper, I used all the data from the 1967/1968 season by using statistical models that can accommodate differing survey duration.
...
I created eight versions of the Richards equation, based on how $P_1$, $P_2$, $S_1$, and $S_2$ were defined with respect to time (Table \@ref(tab:model-definitions)). The $N_{max}$ parameter was assumed to be year-specific and $K$ was assumed constant across all years. Each parameter had a hyper-distribution of either a Gaussian, uniform, or gamma distribution. Uniform distributions were used for the prior distributions of hyperparameters (Appendix Table \@ref(tab:hyperparameter-table)).
...
```{r model-definitions, echo=F, message=F, tab.cap=paste0("Definitions of eight models. Four parameters ($P_1$, $P_2$, $S_1$, $S_2$) were tested for time dependence. $P_1 = P_2 = P[Year]$, means that there was one $P$ for each year.") }
knitr::kable(model.defs, col.names = c("Model", "$P_1$", "$P_2$", "$S_1$", "$S_2$" ), escape = FALSE)

```

The Richards equation provided the daily mean number of gray whales passing through the observation point. As it was done in the Durban et al. approach, the daily number of whales passing through the observation point was modeled with a Poisson distribution:
...
The binomial model was used to approximate the sampling process, as it was done in the Durban et al. approach:
...
hyper.params <- data.frame(params = c("Max", "S1", "S2", "P", "K", "OBS", "BF", "VS", "mean.Prob",
...
The new approach was used to analyze the entire dataset from the 1967/1968 season onward, and the results were compared to the estimates reported in Eguchi et al. (2025). In order to apply the new approach to historical data, sightings data between the 1967/1968 and 2005/2006 seasons were obtained from the ERAnalysis package (<https://github.com/jlaake/ERAnalysis>, Laake 2012). An R script was written to convert the sightings data in the package to an input object required for the new analysis. No raw data were available for the 2006/2007 season, so it was excluded from this analysis. Data from the 2007/2008 season to the 2024/2025 season were extracted from raw data files using another R script (available at the GitHub repository). All shifts of at least 60 minutes with the Beaufort sea state of \< 5 and the visibility code of \< 5 were included in the analysis. Laake et al.’s approach was applied to the entire dataset after formatting new data from the 2009/2010 season into the input object for Laake et al.’s approach. Because no data to match sightings of pods between the two stations in the 2009/2010 and 2010/2011 seasons, only "naive" estimates in the Laake's approach were computed. 

The new models were fitted to the data using JAGS (Plummer) in the R statistical environment, using the *jagsUI* package (Kellner 2024). For each model, five MCMC chains were run for 550,000 steps each. The first 500,000 samples of each chain were discarded as “burn-in.” The remaining posterior samples were thinned by taking every 100 samples, resulting in 2500 posterior samples for inference. The convergence of MCMC was assessed using the rank-normalized R-hat statistic [@vehtari2021]. Goodness-of-fit was determined via Pareto-k statistics [@vehtari_practical_2017] via the *loo* package (Vehtari et al. 2024). The performance of models were compared using the LOOIC statistic via the *loo* and *rstanarm* (Goodrich et al. 2024) packages.
...
JAGS models and R scripts to conduct the analysis, including data extraction and executing the JAGS models, are available at the GitHub repository ().
...
# I thought using lapply would be faster but the for loop option below was slighly faster than
# the lapply... 
# t.1 <- Sys.time()
...
The eight models were fitted to the sightings data over 32 seasons. Based on LOOIC and Pareto-k statistics, Model `r out.table$model[1]` was identified as the best-fitting model (Table \@ref(tab:model-table)). Although Model `r model.ID[which(prop.big.Rhat == min(prop.big.Rhat))]` had the lowest proportion of the monitored parameters with the R-hat statistics \< 1.01 (Table \@ref(tab:model-table)), after visual inspection of trace plots for parameters with high Rhat statistics (Appendix Figures \@ref(fig:Figure-trace-1), \@ref(fig:Figure-trace-2), and \@ref(fig:Figure-trace-3)), I concluded that Model `r out.table$model[1]` was the most appropriate for inference. The model consists of year-specific $P$ ($P_1 = P_2$), $S_1$, and $S_2$ parameters (Table \@ref(tab:model-definitions)).

```{r model-table, echo=F, message=F, tab.cap="Comparison of eight models."}
knitr::kable(out.table, col.names = c("Model", "$\\Delta$LOOIC", "# monitored parameters", "# Rhat > 1.01", "% Rhat > 1.01", "# Pareto-K > 0.7", "% Pareto-K > 0.7" ), digits = 2, escape = FALSE)
...
Estimated abundances from the new approach generally followed the same trend as the previous analyses, with a few notable differences (Figure \@ref(fig:Figure-N-hats)). For many years, Laake's and the new approach provided almost identical estimates. The new and Laake et al. approaches produced lower estimates than Durban et al.’s approach in most years. For the majority of the estimates, the 95% credible and confidence intervals overlapped significantly.

```{r Figure-N-hats, echo=FALSE, message=FALSE, fig.cap = "Estimated annual abundance and their 95% CIs using Laake's method (blue), Durban's method (red), and the new approach (Eguchi M5, green). Laake's estimates prior to 2007 and Durban's estimates are found in Eguchi et al. (2025). Laake's estimates since the 2009/2010 season were computed using the scripts found in the *ERAnalysis* package."}
...
gam.preds.2006.Eguchi <-predict(gam.2006.Eguchi, newdata = pred.data.2006, se.fit = TRUE)
gam.preds.all.Eguchi <-predict(gam.all.Eguchi, newdata = pred.data.all, se.fit = TRUE)

gam.preds.2006.Laake <-predict(gam.2006.Laake, newdata = pred.data.2006, se.fit = TRUE)
gam.preds.all.Laake <-predict(gam.all.Laake, newdata = pred.data.all, se.fit = TRUE)

gam.preds.2006.Durban <-predict(gam.2006.Durban, newdata = pred.data.2006, se.fit = TRUE)
...
GAMs fitted to the estimated abundance between the 2006/2007 and 2024/2025 seasons indicated that all three approaches provided comparable results regarding the general trend with a main difference at the beginning of the timeseries (Figure \@ref(fig:Figure-gam-fits)). For the 2006/2007 season, the abundance estimate with the new approach was notably lower than those from the other two methods (Figure \@ref(fig:Figure-N-hats)). This difference resulted in a significant difference in estimated intercept (the average of abundance estimates) between the new (`r format(coef(gam.2006.Eguchi)["(Intercept)"], scientific = F)` $\pm$ `r signif(summary(gam.2006.Eguchi)$se["(Intercept)"], 2)`) and the other two approaches (Laake: `r format(coef(gam.2006.Laake)["(Intercept)"], scientific = F)` $\pm$ `r signif(summary(gam.2006.Laake)$se["(Intercept)"], 2)`, Durban: `r format(coef(gam.2006.Durban)["(Intercept)"], scientific = F)` $\pm$ `r signif(summary(gam.2006.Durban)$se["(Intercept)"], 2)`). 

For the linear model ($log(Nhat) = time + \epsilon$) fitted to the point estimates from the 2014/2015 season to the 2024/2025 season revealed a statistically indistinguishable annual rate of decline among the three approaches (Laake: `r signif(lm.Laake$coefficients[2],1)` $\pm$ `r signif(coef(summary(lm.Laake))["time", "Std. Error"], 2)`. Durban: `r signif(lm.Durban$coefficients[2],1)` $\pm$ `r signif(coef(summary(lm.Durban))["time", "Std. Error"], 2)`, Eguchi: `r signif(lm.Eguchi$coefficients[2],1)` $\pm$ `r signif(coef(summary(lm.Eguchi))["time", "Std. Error"], 2)`).

```{r Figure-gam-fits, echo=FALSE, message=FALSE, fig.cap = "Fitted generalized additive models to estimated abundance and their approximate 95% CIs."}
...
The new approach is also adaptable and can incorporate additional information as it becomes available. For example, data from uncrewed aircraft systems (UAS) are being used to estimate precision and bias in group size estimates made by visual observers. If a correction function can be derived from these UAS data, it could be integrated into the observation likelihood to adjust each sighting. Finally, this abundance estimation model could be incorporated into a more holistic population model, such as the one developed by [@stewart_boom-bust_2023].

In summary, the new approach provides a simpler, faster, and more statistically robust algorithm to estimate the abundance of gray whales from the long-term visual surveys conducted at Granite Canyon, CA, while producing results that are consistent with previous methods.
...
```{r hyperparameter-table, echo=FALSE, message=FALSE, tab.cap="Definitions of hyperdistributions and their parameters."}
knitr::kable(hyper.params, col.names = c("Parameter", "Distribution"), escape = FALSE)
