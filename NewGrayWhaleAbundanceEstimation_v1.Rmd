...
I created eight versions of the Richards equation, based on how $P_1$, $P_2$, $S_1$, and $S_2$ were defined with respect to time (Table \@ref(tab:model-definition)). The $N_{max}$ parameter was assumed to be season-specific and $K$ was assumed to be constant across all seasons. Each parameter had a hyper-distribution of either a Gaussian, uniform, or gamma distribution. Uniform distributions were used for the prior distributions of hyperparameters (Appendix Table A`r run_reference("hyperparameter-table")`).
...
The new models were fitted to the data using JAGS (Plummer) in the R statistical environment, using the *jagsUI* package (Kellner 2024). For each model, five MCMC chains were run for 550,000 steps each. The first 500,000 samples of each chain were discarded as “burn-in.” The remaining posterior samples were thinned by taking every 100 samples, resulting in 2500 posterior samples for inference. The convergence of MCMC was assessed using the rank-normalized `r R_hat` statistic [@vehtari2021]. Goodness-of-fit was determined via Pareto-k statistics [@vehtari_practical_2017] implemented in the *loo* package (Vehtari et al. 2024). The performance of models was compared using the LOOIC (Leave-One-Out Information Criteria) statistic via the *loo* and *rstanarm* (Goodrich et al. 2024) packages.

To compare the three approaches (Laake et al., Durban et al., and the new approach), I fitted generalized additive models (GAMs) to abundance estimates between the 2006/2007 and 2024/2025 seasons. The rate of recent decline in the estimated abundance was compared by fitting linear models to the abundance estimates between the 2015/2016 and 2024/2025 seasons.
...
# Determine effective sample sizes
ESS <- coda::effectiveSize(jm.out$jm$samples)
...
ESS.bulk.all[order(ESS.bulk.all$ess_bulk),] %>% data.frame -> tmp.1
ESS.tail.all[order(ESS.tail.all$ess_tail),] %>% data.frame -> tmp.2

# mcmc_pairs(jm.out$jm$samples, pars = c("S1", "S2", "P[1]", "K", "Max[1]"))
# mcmc_pairs(jm.out$jm$samples, pars = c("S1[1]", "S2[1]", "P[1]", "K", "Max[1]"))

# Low ESS.bulk and ESS.tail were found so Claude recommended to look at 
# correlations among Richareds function parameters.
# mcmc_pairs(jm.out$jm$samples, pars = c("S1[1]", "S2[1]", "P[1]", "K", "Max[1]"))
# posterior.matrix <- as.matrix(jm.out$jm$samples)
# param.S1 <- posterior.matrix[,"S1[1]"]
# param.S2 <- posterior.matrix[,"S2[1]"]
# cor(param.S1, param.S2)
# param.P1 <- posterior.matrix[,"P[1]"]
# cor(param.S1, param.P1)
# cor(param.S2, param.P1)

posterior <- jm.out$jm$sims.list
N_samples <- posterior$Corrected.Est[,1]
logit_p_samples <- posterior$mean.prob
p_samples <- exp(logit_p_samples)/(1 + exp(logit_p_samples))
p.N.df <- data.frame(p = p_samples,
                     N = N_samples)

ggplot(p.N.df) + 
  geom_point(aes(x = p, y = N))

#plot(p_samples, N_samples)

# The plot looks fine, no strong p-N relationship. 

# High correlations between S1[1] and P[1] and between S2[1] and P[1] were found (abs(r) > 0.9)
# So Claude suggested to make the prior for P narrower. 
...
ESS.Rhat.big <- ESS[params.Rhat.big]

ESS.small <- ESS[ESS < 400]
Rhat.ESS.small <- new.Rhat[names(ESS.small)]
...
if (save.fig) {
  save.fig.fcn(fig = p.trace.1, 
               file.name = paste0("figures/M", ver, "_trace_1.png"), 
               replace = T, 
               dpi = dpi.set, device = "png",
  ...
  save.fig.fcn(fig = p.trace.2, 
               file.name = paste0("figures/M", ver, "_trace_2.png"), 
               replace = T, 
               dpi = dpi.set, device = "png",
               height = 10, width = 10, units = "in",
               bg = "white")
  ...
  save.fig.fcn(fig = p.trace.3, 
               file.name = paste0("figures/M", ver, "_trace_3.png"), 
               replace = T, 
               dpi = dpi.set, device = "png",
               height = 10, width = 10, units = "in",
               bg = "white")
  ...
  save.fig.fcn(fig = p.Pareto.k, 
               file.name = paste0("figures/High_Pareto_k_M", ver, ".png"), 
               replace = T, 
               dpi = dpi.set, device = "png",
               height = 10, width = 10, units = "in",
               bg = "white")
...
# Look at correlations between B0 and other coefficients:
mean.prob.col <- grep("mean.prob", dimnames(jm.out$jm$samples[[1]]))
mean.prob.samples <- lapply(jm.out$jm$samples, FUN = function(x){x[, mean.prob.col]})
mean.prob.p.samples <- lapply(mean.prob.samples, FUN = function(x){1/(1 + exp(-x))})
...
Effective sample sizes (ESS) for the best model (Model `r ver`) with large rank-normalized Rhat statistics ranged from `r min(ESS.Rhat.big)` to `r max(ESS.Rhat.big)`, with the mean of `r mean(ESS.Rhat.big` and SD of `r sqrt(var(ESS.Rhat.big))`. Although `r length(ESS.Rhat.big < 400)` ESS were small (ESS < 400), visual inspections of trace plots indicated acceptable convergence of posterior samples (Appendix Figures `r officer::run_word_field("REF Figure-trace-1 \\h")`, `r officer::run_word_field("REF Figure-trace-2 \\h")`, and `r officer::run_word_field("REF Figure-trace-3 \\h")`). High Rhat statistics were found for Max (n = `r length(grep("Max", names(ESS.Rhat.big)))`), S1 (n = `r length(grep("S1", names(ESS.Rhat.big)))`), S2 (n = `r length(grep("S2", names(ESS.Rhat.big)))`), P (n = `r length(grep("P\\[", names(ESS.Rhat.big)))`), and the observer random effects (n = `r length(grep("OBS.RF", names(ESS.Rhat.big)))`). 
...
tmp.1 %>%
  ...
  rbind(Nhat.) %>%
  mutate(Method2 = factor(Method, levels = c("Laake", "Durban", "Eguchi M5"))) %>%
...
pred.2006 <- rbind(pred.2006.Eguchi, pred.2006.Durban, pred.2006.Laake) %>%
  ...
  mutate(Method = factor(Method1, levels = c("Laake", "Durban", paste0("Eguchi M", ver))))
...
pred.all <- rbind(pred.all.Eguchi, pred.all.Laake) %>%
  rename(Method1 = Method) %>%
  mutate(Method = factor(Method1, levels = c("Laake", paste0("Eguchi M", ver))))
...
The new approach offers several advantages over the method of Durban et al. First, it eliminates the use of the “cut” function, which has been identified as statistically inappropriate (Plummer 2015). Second, the model is coded in JAGS, a more contemporary and widely used Bayesian modeling language. The new model in JAGS runs significantly faster than Durban et al.'s model in WinBUGS. The analysis of the entire 32-year dataset took approximately `r signif(jm.out$Run_Time, 2)` hrs per model with the new approach, compared to `r signif(WinBugs.out$Run_Time, 3)` hours for an 11-year analysis using WinBUGS. Third, the new framework allows for formal model selection and goodness-of-fit assessment using modern criteria, such as LOOIC and the Pareto-k statistic, which were not part of the original Durban et al. analysis. Fourth, while retaining the N-mixture modeling framework, the new approach is more straightforward in its implementation. In the Durban et al.'s approach, effort (i.e., watch duration) was used to adjust the available number of whales during the shift. This approach implicitly makes an assumption that the number of whales per unit time is constant throughout the day. In the new approach, effort was used to offset the detection probability, which does not require the assumption of a constant passage rate.  
