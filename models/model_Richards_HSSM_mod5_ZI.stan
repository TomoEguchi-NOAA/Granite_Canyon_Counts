...
// probability is modelled as a function of the expected count rather than
// given a season-specific parameter:
//
//     logit(pi_i) = zi_a + zi_b * (log kappa_i - mean(log kappa))
//
// zi_b < 0 puts the excess zeros where the curve predicts few whales, which is
// what the diagnostics show. zi_mode: 0 = off, 1 = constant pi (zi_b fixed at
// 0), 2 = count-dependent pi. Two parameters at most.
//
// NOTE: the zero excess and the too-heavy upper tail are the SAME problem --
...
data {
  ...
  // ---- Richards shape exponent, gamma ----------------------------------
  // theta = exp(gamma) governs the SHAPE of each Richards transition. The
  // tail of a limb decays with e-folding time S * theta, so smaller gamma
  // => thinner tails => less mass extrapolated into unobserved days.
  // gamma is identified by shoulder curvature where data are dense, NOT by
  // the tails, where S and theta are confounded through their product.
  // Keep gamma SHARED across seasons; a year-specific gamma is not identified.
  // HARD CONSTRAINT: 2*exp(gamma) - 1 > 0, i.e. gamma > -log(2) = -0.6931.
  real<lower=0> gamma_fix;          // 1.0 reproduces the JAGS model exactly
  int<lower=0, upper=1> estimate_gamma;
  int<lower=0, upper=1> separate_gamma;   // 1 = distinct gamma for C1 and C2
  real<lower=-0.6931> gamma_lower;        // e.g. -0.60, safely inside the bound
  real gamma_prior_mu;                    // e.g. 1.0
  real<lower=0> gamma_prior_sd;           // e.g. 1.0
  ...
  // ---- Zero inflation ---------------------------------------------------
  int<lower=0, upper=2> zi_mode;    // 0 = none, 1 = constant pi, 2 = pi(kappa)
  real zi_a_mu;                     // prior mean for the intercept, e.g. -3
...
parameters {
  ...
  vector[n_observer] alpha_raw;                    // cell means, NO global intercept
  ...
  // Length 0 (fixed), 1 (shared), or 2 (separate for C1 and C2)
  vector<lower=gamma_lower>[estimate_gamma ? (separate_gamma ? 2 : 1) : 0] gamma_free;
...
transformed parameters {
  ...
  real g1 = gamma_fix;
  real g2 = gamma_fix;
  real log_gt1;  real ie1;
  real log_gt2;  real ie2;
  ...
  vector[n_observer] alpha = logit_p0 + sigma_Obs * alpha_raw;
	
  if (estimate_gamma) {
    g1 = gamma_free[1];
    g2 = separate_gamma ? gamma_free[2] : gamma_free[1];
  }
  log_gt1 = log(2 * exp(g1) - 1);  ie1 = 1.0 / exp(g1);   // C1, descending limb
  log_gt2 = log(2 * exp(g2) - 1);  ie2 = 1.0 / exp(g2);   // C2, ascending limb
...

  {
    ...
    for (y in 1:n_year) {
      ...
      for (t in 2:(n_days - 1)) {
        ...
        real log_C1 = -ie1 * log1p_exp(log_gt1 - inv_S1 * p_minus_t1);
        real log_C2 = -ie2 * log1p_exp(log_gt2 + inv_S2 * p_minus_t2);
...
model {
  ...
  // Changed to alpha_raw: alpha    ~ normal(logit_p0, sigma_Obs);           // JAGS: dnorm(1.39, tau.obs)
  ...
  if (estimate_gamma) gamma_free ~ normal(gamma_prior_mu, gamma_prior_sd);
  if (zi_mode > 0)  zi_a ~ normal(zi_a_mu, zi_a_sd);
  if (zi_mode == 2) zi_b ~ normal(0, zi_b_sd);
  ...
  // ---- Observation model -------------------------------------------------
  {
    ...
    if (zi_mode == 0) {
      ...
      // Zero-inflated: with prob pi the count is a structural zero; otherwise
      // it is drawn from the count distribution (which can also give zero).
      real lk_bar = mean(log_kappa);
      vector[N_flat] logit_pi = rep_vector(zi_a[1], N_flat);
      if (zi_mode == 2) logit_pi += zi_b[1] * (log_kappa - lk_bar);

      for (i in 1:N_flat) {
        real lp1 = log_inv_logit(logit_pi[i]);        // log pi
        real lp0 = log1m_inv_logit(logit_pi[i]);      // log(1 - pi)
        real ll  = likelihood_NB
                 ? neg_binomial_2_log_lpmf(n[i] | log_kappa[i], phi[1])
                 : poisson_log_lpmf(n[i] | log_kappa[i]);
        if (n[i] == 0) target += log_sum_exp(lp1, lp0 + ll);
        else           target += lp0 + ll;
      }
    }
  }
...
generated quantities {
  ...
  real pi_mean = zi_mode > 0 ? inv_logit(zi_a[1]) : 0.0;
  real theta1 = exp(g1);          // shape parameter of the descending limb
  real theta2 = exp(g2);          // shape parameter of the ascending limb
