...
# no shift was picked up by V1

FinalData.Both %>% 
  filter(time.steps == difs.1[idx, "time.step"]) %>% 
  filter(v == "V2") -> tmp.V2

# no results returned in V1... 
Data_Out.v2 %>% 
  filter(time.steps == difs.1[idx, "time.step"]) 
...
V2 has one more period and 8 more whales
...
# V2 picked up 3, 4, 5, and 6. V2's 4, 5, and 6 are the same as V1's 6, 7, and 8, 
