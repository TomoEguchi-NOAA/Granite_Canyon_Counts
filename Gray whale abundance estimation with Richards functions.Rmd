...
max.r.hat <- max(unlist(lapply(jm.out$jm$Rhat, 
                               ...
                               na.rm = T)))
jm <- jm.out$jm
param.names <- c("S1", "S2", "K", "P")
true.values <- c(S1, S2, K, P)
...
# extract.samples.jagsUI <- function(jm,varname){
...
#     samples.list[[k]] <- data.frame(name = par.names[col.idx[k]],
#                                     value = unlist(samples),
#                                     iteration = rep(1:length(samples[[1]]),
#                                                     times = length(samples)),
#                                     chain = rep(1:length(samples),
#                                                 each = length(samples[[1]])))
#   }
# 
#   return(samples.list)
# }
...
plots.trace <- function(jm, params){
  ...
  for (i in 1:nrow(params)){
    tmp <- mcmc_trace(jm$samples, params[i, "name"])
    tmp$data$true.value <- params[i, "value"]
    p.tmp <- ggplot()  +
      geom_path(data = tmp$data, 
                aes(x = iteration, y = value, color = chain)) + 
      scale_color_brewer("blue") +
      ylab(params[i, "name"]) +
      geom_hline(aes(yintercept = first(tmp$data$true.value)),
                 color = "red", size = 1.2)
...
p.trace <- plots.trace(jm.out$jm, params = params)



plots.dens <- function(jm, param.names, true.values){
  out.list <- list()
  for (k in 1:length(param.names)){
    out.list[[k]] <- mcmc_dens(jm$samples, param.names[k]) +
      geom_vline(aes(xintercept = true.values[k]),
                 color = "red", size = 1.2)
  }
...
p.S1.trace <- mcmc_trace(jm.out$jm$samples, "S1") +
  geom_hline(aes(yintercept = S1), color = "red", size = 1.5)

mcmc_dens(jm.out$jm$samples, "S1") +
  geom_vline(aes(xintercept = S1), color = "red", size = 1.2)
