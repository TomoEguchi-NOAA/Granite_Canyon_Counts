...
In order to compare the new method to the current method (Durban et al.), I analyzed the same simulated dataset with the analytical approach in Durban et al. WinBUGS returns error (undefined real result), which appears to be caused by computing log(0). I can't find the problem. I'll move on to comparing results of real datasets. 
...
if (!file.exists(paste0("RData/WinBUGS_sim_", Sys.Date(), ".rds"))){
  ...
  bm <- bugs(data = BUGS.data,
             inits = data.worked$BUGS.inits, #BUGS.inits.1,
             parameters = data.worked$parameters,
             model.file = BUGS.model,
             n.chains = MCMC.params$n.chains,
             n.iter = MCMC.params$n.samples,
...
# 	.x	REAL	-inf
#
# There must be log(0) somewhere...
```


array index is greater than array upper bound for u


Analyzing the same data from WinBUGS analysis of real data:

```{r new-analysis, echo=FALSE}
data.worked <- readRDS("RData/BUGS_data_runs.rds")
out.file.name <- "RData/JAGS_pois_binom_results.rds"

jags.data.real <- list(  n = data.worked$BUGS.data$n, 
                         n.station = rep(1, dim(data.worked$BUGS.data$n)[3]),
                         n.year = dim(data.worked$BUGS.data$n)[3],
                         n.obs = data.worked$BUGS.data$n.obs,
                         periods = data.worked$BUGS.data$periods,
                         obs = data.worked$BUGS.data$obs,
                         vs = scale(data.worked$BUGS.data$vs),
                         bf = scale(data.worked$BUGS.data$bf),
                         watch.prop = (data.worked$BUGS.data$Watch.Length*24*60)/540,
                         day = data.worked$BUGS.data$day)
...
No good convergence... need to look into this a bit more. 2022-10-07
...
#                      value = true.values)
# fix these:
param.names <- c("S1[1]", "S1[2]","S1[3]", "S1[4]","S1[5]", "S1[6]","S1[7]", "S1[8]", 
                 "S2[1]", "S2[2]", 
                 "K", 
                 "P[1]", "P[2]", 
                 "max[1]", "max[2]", 
