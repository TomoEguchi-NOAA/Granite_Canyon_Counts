...
N.mean[,1] <- dgamma(1:90, 3, 0.07) * 5000
N.mean[,2] <- Richards_fcn(d = Days, S1 = S1, S2 = S2, K = K, P = P, min = 0, max = Max)
N.mean[90,1:2] <- 0
...
if (!file.exists(out.file.name)){
  ...
  obs <- obsd.n <- array(dim = c(length(Days), 1, length(Season)))
  for (y in 1:length(Season)){
    ...
    watch.prop <- bf <- vs <- day <- array(dim = c(nrow(Final_Data), 1))
    ...
    for (k in 1:nrow(Data_True.N)){
      obsd.n[k, 1, y] <- rbinom(n = 1,  
                                size = Data_True.N$N.true[k], 
                                prob = Data_True.N$Binom.Prob[k])
      
      obs[k,1,y] <- Data_True.N$obs.ID[k]
      watch.prop[k, y] <- Data_True.N$watch.prop[k]
      
      bf[k, y] <- Data_True.N$bf[k]
      vs[k, y] <- Data_True.N$vs[k]
      day[k, y] <- Data_True.N$Day[k]
    }
...
N.hats <- data.frame(Season = Season, #rep(seasons, each = 90),
                     Day = 1:90, #rep(1:90, times = 4),
                     Mean = as.vector(jm.out$jm$mean$N),
                     LCL = as.vector(jm.out$jm$q2.5$N),
                     UCL = as.vector(jm.out$jm$q97.5$N),
                     true.N = as.vector(N$N.true))
...
if (save.fig)
  cowplot::save_plot(p.trace.all, 
                     filename = "figures/trace_plots.png",
...
```{r Figure-trace, echo=FALSE, message=FALSE, fig.cap="Trace plots of eight parameters. Red horizontal lines indicate the true values."}

knitr::include_graphics("figures/trace_plots.png")
