...
N.mean <- dgamma(1:90, 3, 0.07) * 5000
...
out.file.name <- "RData/Pois_Binom_sim_Gamma_results.rds"

if (!file.exists(out.file.name)){
  # I use the real data to emulate sampling and sighting conditions
  Season = "2020"
  tmp <- readRDS(paste0("RData/V2.1_Aug2022/out_", Season, "_Tomo_v2.rds"))
...
param.names <- c("S1", "S2", "K", "P", 
                 "mean.prob", "BF.Fixed", "VS.Fixed", "max")
...
if (save.fig){
  for (k in 1:nrow(params)){
    ggsave(plot = p.trace[[k]] +
             geom_hline(aes(yintercept = params[k, "value"]),
                        color = "red", size = 1.2),
           filename = paste0("figures/", params[k, "name"], "_trace.png"),
           
           dpi = 600, 
           device = "png")
    
    ggsave(filename = paste0("figures/", params[k, "name"], "_dens.png"),
           plot = p.dens[[k]] +
             geom_vline(aes(xintercept = params[k, "value"]),
                        color = "red", size = 1.2),
           dpi = 600, device = "png")
