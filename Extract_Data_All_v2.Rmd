...
for (c in 1:length(unique.dates)){
  ...
  Data_Out.list[[c]] <- do.call("rbind", Output.list)
  all.sightings.list[[c]] <- do.call("rbind", all.sightings)
  all.effort.list[[c]] <- do.call("rbind", all.effort)
...
Data4Laake <- do.call("rbind", all.sightings.list) %>%
  mutate(key.1 = paste0(as.Date(Date, format = "%m/%d/%Y"), "_", Shift, "_", Group_ID)) %>%
  transmute(Date = Date,
            Time_PST = Time,
            key = key.1,
            Group_ID = Group_ID,
...
# 2023-11-29 Double check on what the "key" should be in the effort file.
