...
The recent version as of December 2023 also creates input files for Laake's approach. That approach requires double-observer data, which are only available for 2010 and 2011 in the recent data. Years without double observers are adjusted by using the average observation probability from years with double observers. Use LaakeAnalysis_NewData.R for the analysis. There is not enough information in 2010 and 2011 to run Laake's analysis. Laake's analysis requires a Match dataframe, which indicates whether or not each pod was recorded by primary and secondary teams. No such information is available in the raw data for 2010 and 2011. 
...
# The following line will make the script stop when a warning returns.
options(warn = 2)

save.output <- FALSE # TRUE
...
YEAR <- 2015 #2026 #2008 #10 #Enter the year of the data files
...
shift_dur_min <- 90   #15 # 90 minutes or anything else. 
grace_min <- 30 # 5 # 60 #       # this makes it minimum of shift_dur_min - grace_min
...
# I fixed the first line issue in Dec 2023. These have been mostly fixed May 2026.
...
for(ff in 1:length(FILES)){ 
  ...
  data.list[[ff]] <- get.data(in.dir, YEAR, FILES = FILES, ff = ff)
}
...
Create data for Laake's approach:

```{r}
# I think I fixed the decimal hour issues in 2010 and 2008 data files. But, keep it 
# just in case:
# if (YEAR == 2010){
...
SHIFT IDENTIFICATION IS WRONG. GO BACK TO GET.SHIFT FUNCTION AND FIX THE PROBLEM FOR 2008. 2025-09-19
THESE ERRORS WERE DUE TO DATA ENTRY "ERRORS." THEY HAVE BEEN FIXED. SEE Extract_Data_2008.Rmd FOR DETAILS.
