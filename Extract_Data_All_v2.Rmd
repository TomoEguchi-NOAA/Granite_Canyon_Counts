...
# YEAR should be one of 2009, 2010, 2015, 2016, 2020, 2022, 2023 (and beyond)
YEAR <- 2010 #Enter the year of the data files

# For 2009 (maybe in the previous years too but I don't have any of those files),
...
for (c in 1:length(unique.dates)){
  ...
  n.stations <- length(unique(data$ff))
...
# Start from here 2023-12-04
# Data need to be saved differently depending on whether or not there was a secondary
# station. 

# For primary:
Data_Out_1 <- do.call("rbind", Data_Out.list.1)
Data4Laake_1 <- do.call("rbind", all.sightings.list.1) %>%
...
Effort4Laake_1 <-  do.call("rbind", all.effort.list.1) %>%
  mutate(key.1 = paste0(as.Date(Date, format = "%m/%d/%Y"), "_", Shift, "_", key),
         year.shift = paste0(as.Date(Date, format = "%m/%d/%Y"), "_", Shift)) %>%
  transmute(Date = Date,
            Start.year = YEAR - 1,
            key = key.1,
            shift = shift,
            year.shift = year.shift,
            npods = npods,
            nwhales = nwhales,
            beaufort = bft,
            vis = vis,
...
Effort4Laake_1 %>% 
  mutate(seq = 1:nrow(Effort4Laake),
         watch.key = paste0(Start.year, "_", seq)) %>%
  select(-c(seq)) -> Effort4Laake_1
...
saveRDS(out.obj, file = paste0("RData/V2.1_Nov2023/out_", YEAR, "_Tomo_v2.rds"))
saveRDS(out.obj, file = paste0("RData/V2.1_Nov2023/out_", YEAR, "_min", min_dur, "_Tomo_v2.rds"))
