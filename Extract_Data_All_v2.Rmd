...
seq.year.shift <- data.frame(year.shift = unique(Effort4Laake$year.shift),
                             Start.year = YEAR - 1,
                             seq = 1:length(unique(Effort4Laake$year.shift))) %>%
  mutate(watch.key = paste0(Start.year, "_", seq)) %>%
  select(-c(seq, Start.year))

Effort4Laake %>%
  left_join(seq.year.shift, by = "year.shift") -> Effort4Laake
