...
for(ff in 1:length(FILES)){ 
    ...
    for(i in 1:(length(Shifts)-1)){
      ...
      BeginDay <- mdy(data[Shifts[i],3]) - mdy("11/30/2015") # Days since Nov 30th
      BeginHr <- (hour(hms(data[Shifts[i],4])) + 
                    (minute(hms(data[Shifts[i],4]))/60)) # Decimal hour of shift start time
      NextBeginHr <- (hour(hms(data[Shifts[i+1],4])) + 
                        (minute(hms(data[Shifts[i+1],4]))/60)) # Decimal hour of next shift start time
...
for(ff in 1:length(FILES)){ 
  ...
  data <- read.table(text = dataLines[-COMMENTS], fill=T, na.strings = "", stringsAsFactors = F, col.names = paste0("V",1:16))
  }else{
  data <- read.table(text = dataLines, fill=T, na.strings = "", stringsAsFactors = F, col.names = paste0("V",1:16))
  }
  # Read in the file, fill blanks with NA, skip the first line (ONLY SKIP IF FILES HAVE 'EDITED FOR' LINE TO BEGIN)
  #data <- read.table(paste0(getwd(),"/2016 Edited for JWD/",FILES[ff]), fill=T, skip=1, na.strings = "", stringsAsFactors = F)
