...
This runs but it is slow. It took 1.6 minutes to run the following MCMC setup:
...
if (!file.exists(paste0("RData/WinBUGS_sim_", run.date, ".rds"))){
  ...
  BUGS.out <- readRDS(paste0("RData/WinBUGS_sim_", run.date, ".rds"))
...
```{r WinBUGS-results, echo=FALSE}
seasons <- c("2020", "2022")

# Extract estimated counts
Daily.Est <- BUGS.out$bm$sims.list$Daily.Est
sp <- BUGS.out$bm$sims.list$sp
com <- BUGS.out$bm$sims.list$com
Corrected.Est <- BUGS.out$bm$sims.list$Corrected.Est
