...
$$ n_{i,j,t} \sim BIN(N_{j,t}, p_{i,j,t}) $$ 

The binomial $N_{j,t}$ parameter is the unknown total number of gray whales passing through the study area during the watch period $j$ in the year $t$.

The detection probability was modeled as a function of visibility (V), Beaufort scale (B), and observers (O).

$$ logit(p_{i,j,t}) = logit(p_0) + I_{V} \beta_{V} VS_{j,t} + I_{B} \beta_{B} B_{j,t}+ I_{O} \mathbf{\beta}^{O_{i,j,t} = o}_{O} $$
...
where $n_{d_t, s, y}$ is the observed number of gray whales during the watch period $d$ of the $t$-th day of the season $y$ from the station $s$, $N_{t, y}$ is the number of gray whales that migrated through the sampling area during the $t$-th day of season $y$, $p_{d_t, s, y}$ is the sighting probability of the station $s$ during the watch period $d$ of the $t$-th day of the season $y$, and $\theta_{d_t,y}$ is the fractional duration of the watch period $d_t$ to the total possible (9 hrs), e.g., 3 hrs equals to 3/9 = 0.3.
...
$$ logit(p_{d_t, s, y}) = \beta_0 + I_{B} * \beta_{B} * B_{d_t, y} + I_{V} * \beta_{V} * V_{d_t, y} + I_{O} * O_{d_t, s, y}  $$
