...
run.date.real <- Sys.Date() #"2022-10-28"   # Change it accordingly
...
if (save.fig){
  for (k in 1:length(param.names)){
    ggsave(plot = p.trace[[k]] + 
             geom_hline(aes(yintercept = params.df[k, "value"]),
                        color = "red", size = 1.2),
           filename = paste0("figures/", param.names[k], "_real_trace_", 
                             dpi.set, "dpi.png"),
    ...
    ggsave(filename = paste0("figures/", param.names[k], "_real_dens_", 
                             dpi.set, "dpi.png"),
           plot = p.dens[[k]],
           dpi = dpi.set, device = "png",
           height = 3, width = 3, units = "in",
...
if (save.fig)
  ggsave(p.mean.N.hats.JAGS, 
         filename = paste0("figures/mean_N_pois_bino_real_", dpi.set, "dpi.png"),
         device = "png", dpi = dpi.set)
...
if (save.fig)
  ggsave(p.N.hats, 
         filename = paste0("figures/N_hats_BUGS_JAGS_real_", dpi.set, "dpi.png"),
         device = "png", dpi = dpi.set)
...
if (save.fig)
  ggsave(p.corrected.N.hats, 
         filename = paste0("figures/corrected_N_BUGS_JAGS_real_", dpi.set, "dpi.png"),
         device = "png", dpi = dpi.set)
...
knitr::include_graphics(paste0("figures/corrected_N_BUGS_JAGS_real_", dpi.set, "dpi.png"))
```
