  ...
  for (k in 1:length(param.names)){
    ggsave(plot = p.trace.sim.JAGS[[k]] + 
             geom_hline(aes(yintercept = params.df[k, "value"]),
                        color = "red", size = 1.2),
           filename = paste0("figures/", param.names[k], "_sim_trace_JAGS.png"),
           
           dpi = 600, 
           device = "png",
           height = 3, width = 3, units = "in")
    
    ggsave(filename = paste0("figures/", param.names[k], "_sim_dens_JAGS.png"),
...
if (save.fig)
  ggsave(p.mean.N.hats.sim.JAGS, filename = "figures/mean_N_sim_JAGS.png",
         device = "png", dpi = 600)
...
if (save.fig)
  ggsave(p.N.hats.sim.JAGS, filename = "figures/N_hats_sim_JAGS.png",
         device = "png", dpi = 600)
...
Convergence was reached for all parameters according to the @\hat{R}$ statistic, where the maximum value was `r signif(max.r.hat, 3)`. However, trace plots of some Richards function parameters did not look ideal (Figure \@ref(fig:Figure-trace-sim-JAGS)). The observed poor conversions of S1, S2, and max parameters might have been caused by the mismatch between the true function (gamma) and assumed (Richards) function, where Richards functions could not match the increase (S2) and decrease (S1) of the gamma function.

For the parameters that associated with sighting probabilities (mean.prob, BF.Fixed, and VS.Fixed), MCMC samples appeared to converge, albeit not around the true values (Figure \@ref(fig:Figure-trace-sim-JAGS)). 
...
knitr::include_graphics("figures/trace_plots_sim_JAGS.png")
...
knitr::include_graphics("figures/mean_N_sim_JAGS.png")
```


```{r Figure-N-sim-JAGS, echo=FALSE, message=FALSE, fig.cap = "Estimated $N$ and their 95% CI (blue ribbon) and the true $N$ in gold."}

knitr::include_graphics("figures/N_hats_sim_JAGS.png")
```


#### Comparison to Durban et al's method

In order to compare the new method to the current method (Durban et al.), I analyzed the same simulated dataset with the analytical approach in Durban et al.  
...
if (save.fig)
  ggsave(filename = "figures/BUGS_daily_sim.png",
...
knitr::include_graphics("figures/BUGS_daily_sim.png")
```
...
param.names <- c("S1[1]", "S1[2]", "S1[3]", "S1[4]",
                 "S1[5]", "S1[6]", "S1[7]", "S1[8]", 
                 "S2[1]", "S2[2]", "S2[3]", "S2[4]",
                 "S2[5]", "S2[6]", "S2[7]", "S2[8]",
                 "P[1]", "P[2]", "P[3]","P[4]", 
                 ...
                 "Corrected.Est[1]", "Corrected.Est[2]","Corrected.Est[3]","Corrected.Est[4]",
                 "Corrected.Est[5]","Corrected.Est[6]","Corrected.Est[7]","Corrected.Est[8]",
                 "Raw.Est[1]", "Raw.Est[2]", "Raw.Est[3]", "Raw.Est[4]",
                 "Raw.Est[5]", "Raw.Est[6]", "Raw.Est[7]", "Raw.Est[8]",
                 "K", 
                 "mean.prob", "BF.Fixed", "VS.Fixed")
...
if (save.fig){
  for (k in 1:length(param.names)){
    ggsave(plot = p.trace[[k]] + 
             geom_hline(aes(yintercept = params.df[k, "value"]),
                        color = "red", size = 1.2),
           filename = paste0("figures/", param.names[k], "_trace.png"),
           
           dpi = 600, 
           device = "png",
           height = 3, width = 3, units = "in")
    
    ggsave(filename = paste0("figures/", param.names[k], "_dens.png"),
           plot = p.dens[[k]],
           dpi = 600, device = "png",
           height = 3, width = 3, units = "in")
...
if (save.fig)
  ggsave(p.mean.N.hats.JAGS, 
         filename = "figures/mean_N_pois_bino.png",
         device = "png", dpi = 600)
...
if (save.fig)
  ggsave(p.N.hats, 
         filename = "figures/N_hats_BUGS_JAGS.png",
         device = "png", dpi = 600)
...
if (save.fig)
  ggsave(p.corrected.N.hats, 
         filename = "figures/corrected_N_BUGS_JAGS.png",
         device = "png", dpi = 600)
...
knitr::include_graphics("figures/N_hats_BUGS_JAGS.png")
```
...
knitr::include_graphics("figures/corrected_N_BUGS_JAGS.png")
```
