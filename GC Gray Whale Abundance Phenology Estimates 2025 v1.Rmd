...
N.hats.2025 <- read_csv(file = "Data/WinBUGS_abundance_2007to2025_v2_min85_100000_2025-02-25.csv",
...
p.1 <- ggplot(whale.groups.daily.df) +
  geom_point(aes(x = DOS, y = cumu.N, color = cumu.Effort)) +
  facet_wrap(~Season.f)
...
Laake.estimates %>%
  ...
  mutate(Method = "Laake") %>%
  rbind(Durban.2015) %>%
  rbind(Eguchi.2024) -> all.estimates.2024
...
Laake.estimates %>%
  ...
  rbind(N.hats.2025 %>% 
          ...
          transmute(Year = Year,
                    ...
                    LCL = total.LCL,
                    UCL = total.UCL,
                    Method = "Durban")) -> all.estimates.2025

all.estimates.2025$Nhat.2024 <- c(all.estimates.2024$Nhat, NA)
...
p.Nhat.2025 <- ggplot(data = all.years.2025.df) + 
...
# Find daily data:
data.2025 <- readRDS(file = "RData/V2.1_Feb2025/out_2025_min85_Tomo_v2.rds")

data.2025$Complete_Data %>% 
  group_by(BeginDay) %>%
...
WinBUGS.results <- readRDS(paste0("RData/WinBUGS_2007to2025_v2_min85_100000_2025-02-23.rds"))
Nhat.2025 <- WinBUGS.results$BUGS_out$mean$Corrected.Est[x] 
SD.Nhat.2025 <- WinBUGS.results$BUGS_out$sd$Corrected.Est[x] 
CV.Nhat.2025 <- SD.Nhat.2025/Nhat.2025
...
estimate.new <- all.estimates.2025 %>% filter(Year == YEAR)
estimate.last.year <- all.estimates.2025 %>% filter(Year == (YEAR-1))
