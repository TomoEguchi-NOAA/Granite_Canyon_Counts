...
This one uses the WinBUGS model with spline only
...
# Change output file names accordingly:
Nhats.filename <- paste0("Data/abundance_", years[length(years)], "_", min.duration, "min_spline.csv")
...
out.v2 <- lapply(years, FUN = function(x) readRDS(paste0("RData/V2.1_Aug2022/out_", x, 
                                                          "_Tomo_v2.rds")))
...
out.file.name <- paste0("RData/WinBUGS_", x, "yr_v2_min", min.duration, "_spline.rds")
#out.file.name <- paste0("RData/WinBUGS_", x, "yr_v2_min", 
#min.duration, "_8weeks_2023_spline.rds")
...
MCMCparams <- list(n.iter = 100, #000,
                   n.thin = 8,#0,
                   n.burnin = 60, #000,
                   n.chains = 5)
...
if (!file.exists(out.file.name)){
  ...
  BUGS_out <- bugs(data = BUGS.data,
                  ...
                  model.file="GW_Nmix_Spline.bugs",
